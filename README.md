...
Версия 1.0.2. Проект — форк [goose](https://github.com/aaif-goose/goose) (Rust + приложение
...
| macOS | `Vasilisa.dmg` (Apple Silicon) — образ с окном, куда перетаскивают приложение; рядом тот же `Vasilisa.app` архивом для автообновления |
| Linux | `vasilisa_1.0.2_amd64.deb` — проверяется на Ubuntu 22.04 и новее |

Сборка macOS подписана сертификатом Developer ID и заверена у Apple — открывается
двойным кликом. Сборка Windows не подписана, SmartScreen при первом запуске потребует
подтверждения; порядок действий описан в
