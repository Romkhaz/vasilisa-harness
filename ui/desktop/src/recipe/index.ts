...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `vasilisa://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith('vasilisa://recipe?config=')) {
      throw new Error('Invalid deeplink format. Expected: vasilisa://recipe?config=...');
    }

    const recipeEncoded = cleanLink.replace('vasilisa://recipe?config=', '');
