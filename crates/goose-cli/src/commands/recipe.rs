...
fn generate_deeplink(
    ...
    match recipe_deeplink::encode(&recipe) {
        Ok(encoded) => {
            let mut full_url = format!("vasilisa://recipe?config={}", encoded);
...
"#;
    ...
    fn test_handle_deeplink_valid_recipe() {
        ...
        assert!(url.starts_with("vasilisa://recipe?config="));
        let encoded_part = url.strip_prefix("vasilisa://recipe?config=").unwrap();
